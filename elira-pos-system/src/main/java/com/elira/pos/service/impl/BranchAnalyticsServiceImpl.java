...
import com.elira.pos.service.BranchAnalyticsService;
...
public class BranchAnalyticsServiceImpl implements BranchAnalyticsService {
    ...
    @Override
    public List<DailySalesDTO> getDailySalesChart(Long branchId, Integer days) {
        ...
        for(int i = 0; i < days; i++) {
            ...
            BigDecimal total = orderRepository.getTotalSalesBetween(branchId, start, end)
    ...
    @Override
    public List<ProductPerformanceDTO> getTopProductsByQuantityWithPercentage(
            Long branchId) {

        List<Object[]> rawData = orderItemRepository.getTopProductsByQuantity(branchId);

        long totalQuantity = rawData.stream()
                .mapToLong(obj->(Long)obj[2]).sum();

        return rawData.stream().limit(5)
                .map(obj ->{
                    String name = (String) obj[1];
                    Long quantity = (Long) obj[2];
                    double percentage = totalQuantity == 0 ? 0 : ((double) quantity / totalQuantity) * 100;
    ...
    public List<CategorySalesDTO> getCategoryWiseSalesBreakdown(Long branchId, LocalDate date) {
        ...
        List<Object[]> rawData = orderItemRepository.getCategoryWiseSales(
                branchId,
                start,
                end
        );
    ...
    public List<PaymentSummary> getPaymentMethodBreakdown(Long branchId, LocalDate date) {
        ...
        double total = rawData.stream()
                .mapToDouble(obj -> (Double) obj[1]).sum();
                ...
                return rawData.stream().map(obj -> {
                    ...
                    return new PaymentSummary(type, amount, count, Math.round(percentage * 10.0) / 10.0);
                }).collect(Collectors.toList());
    ...
    public BranchDashboardOverviewDTO getBranchOverview(Long branchId) {
        ...
        int todayOrders = orderRepository.countOrderByBranchAndDate(branchId, today);

        int yesterdayOrders = orderRepository.countOrderByBranchAndDate(branchId, yesterday);
        ...
        double lowStockGrowth = calculateGrowth(orderGrowth, yesterdayLowStock);
